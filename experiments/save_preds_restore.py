...
if __name__ == "__main__":
    ...
    preds_path = os.path.join(ds_info.base_models_dir,"preds",args.old)
    pa = pickle.load( open( os.path.join(preds_path,"preds_a.p"), "rb" ) )
    pv = pickle.load( open( os.path.join(preds_path,"preds_v.p"), "rb" ) )
    ...
    def single_evaluation():
        ...
        for r0 in ratios:
            ...
            for prop_value in ratios:
                ...
                gt[r0][prop_value] = {}
                if prop_value == 0.5 or prop_value==r0:
                        continue
                for t in range(attack_config.tries):
                    ...
                    if EPOCH:
                        ...
                        adv_p,gt__ = _get_preds_accross_epoch(models_adv_1,loader_vic,preload=bb_attack_config.preload,multi_class=bb_attack_config.multi_class)
                        vic_p,gt_ = _get_preds_accross_epoch(models_vic_1,loader_vic,preload=bb_attack_config.preload,multi_class=bb_attack_config.multi_class)
        ...
        for x in ratios:
            ...
            for t in range(attack_config.tries):
                preds_adv = pa[x][t]
                preds_vic = pv[x][t]
                g_t = ground_truths[x][t]
                #just use the 0.5 on 0.5 prediction from 0.0 case
                if x==0.0:
                    preds_a[0.5][0.5][t] = preds_adv.preds_on_distr_1.preds_property_1
                    preds_v[0.5][0.5][t] = preds_vic.preds_on_distr_1.preds_property_1
                    gt[0.5][0.5][t] = g_t[0]
            
                preds_a[x][0.5][t] = preds_adv.preds_on_distr_1.preds_property_2
                preds_v[x][0.5][t] = preds_vic.preds_on_distr_1.preds_property_2
                gt[x][0.5][t] = g_t[0]
                preds_a[0.5][x][t] = preds_adv.preds_on_distr_2.preds_property_1
                preds_v[0.5][x][t] = preds_vic.preds_on_distr_2.preds_property_1
                gt[0.5][x][t] = g_t[1]
                preds_a[x][x][t] = preds_adv.preds_on_distr_2.preds_property_2
                preds_v[x][x][t] = preds_vic.preds_on_distr_2.preds_property_2
                gt[x][x][t] = g_t[1]
