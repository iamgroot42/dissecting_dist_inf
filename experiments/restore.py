...
if __name__ == "__main__":
    ...
    for t in range(attack_config.tries):
        attacker_obj.set_val(0.5,0.5,t)
    ...
    for prop_value in attack_config.values:
        ...
        gt[prop_value] = {}
        for t in range(attack_config.tries):
            attacker_obj.set_val(prop_value,0.5,t)
            _, loader1 = ds_adv_1.get_loaders(batch_size=BATCH_SIZE)
            _, loader2 = ds_adv_2.get_loaders(batch_size=BATCH_SIZE)
            models_adv_1 = ([ma[t]],[attacker_obj.get_models(train_config,50)])
            attacker_obj.set_val(0.5,prop_value,t)
            ma1 = attacker_obj.get_models(train_config,50)
            attacker_obj.set_val(prop_value,prop_value,t)
            ma2 = attacker_obj.get_models(train_config,50)
            models_adv_2 = ([ma1],[ma2])
