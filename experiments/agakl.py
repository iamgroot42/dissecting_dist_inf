...
if __name__ == "__main__":
    ...
    for prop_value in attack_config.values:
        ...
        for (t, attack_model_path) in zip(range(attack_config.tries), attack_model_paths):
            ...
            adv_for_bb_index, adv_for_meta_index = train_test_split(
                list(range(len(models_adv_1))), test_size=attack_config.num_for_meta)
            ...
            preds_adv_1_combined = PredictionsOnOneDistribution(
                preds_property_1=np.concatenate(
                    (preds_adv_on_1.preds_property_1[adv_for_meta_index], preds_vic_on_1.preds_property_1), 0),
                preds_property_2=np.concatenate(
                    (preds_adv_on_1.preds_property_2[adv_for_meta_index], preds_vic_on_1.preds_property_2), 0)
            )
            preds_adv_2_combined = PredictionsOnOneDistribution(
                preds_property_1=np.concatenate(
                    (preds_adv_on_2.preds_property_1[adv_for_meta_index], preds_vic_on_2.preds_property_1), 0),
                preds_property_2=np.concatenate(
                    (preds_adv_on_2.preds_property_2[adv_for_meta_index], preds_vic_on_2.preds_property_2), 0)
            )
            ...
            attacker_obj = get_attack(
                bb_attack_config.attack_type[0])(bb_attack_config)
            ...
            bb_meta_preds = np.concatenate(
                (bb_and_vic_preds_1[:split_point], bb_and_vic_preds_2[:split_point]), 0)
            bb_preds_vic = np.concatenate(
                (bb_and_vic_preds_1[split_point:], bb_and_vic_preds_2[split_point:]), 0)
            ...
            adv_test = wrap_into_loader(
                [models_adv_1[adv_for_meta_index],
                    models_adv_2[adv_for_meta_index]],
