...
if __name__ == "__main__":
    ...
    for prop_value in attack_config.values:
        ...
        for a in os.listdir(attack_model_path_dir):
            if os.path.isdir(os.path.join(attack_model_path_dir, a)):
                continue
            attack_model_paths.append(a)
        #in case the number of trials doesn't match the # of metaclassifier
        
        attack_model_path=attack_model_paths[0]
        
        print("Ratio: {}".format(prop_value))
        ...
        accs = np.transpose([sigmoid(attacker_obj.eval_attack(
                test_loader=(features_test, test_data[1]),get_preds=True))  for features_test,test_data
