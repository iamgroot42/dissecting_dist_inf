...
from distribution_inference.config import DatasetConfig, WhiteBoxAttackConfig, TrainConfig, CombineAttackConfig
...
if __name__ == "__main__":
    ...
    for prop_value in attack_config.values:
        ...
        for (t, attack_model_path) in zip(range(attack_config.tries), attack_model_paths):
            ...
            labels_adv = np.hstack(
                (np.zeros(models_adv_1.shape[1]), np.ones(models_adv_2.shape[1])))
            labels_vic = np.hstack(
