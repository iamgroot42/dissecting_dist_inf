...
if __name__ == "__main__":
    ...
    for prop_value in attack_config.values:
        for t in range(attack_config.tries):
            preds_path = os.path.join(ds_info.base_models_dir,"comparison","preds",args.pred_name,str(t),str(prop_value))
            preds_adv1 = pickle.load( open( os.path.join(preds_path,"preds_a1.p"), "rb" ) )
            preds_adv2 = pickle.load( open( os.path.join(preds_path,"preds_a2.p"), "rb" ) )
            preds_vic = pickle.load( open( os.path.join(preds_path,"preds_v.p"), "rb" ) )
            gt = pickle.load( open( os.path.join(preds_path,"gt.p"), "rb" ) )
