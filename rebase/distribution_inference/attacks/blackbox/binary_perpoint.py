...
class BinaryPerPointThresholdAttack(Attack):
    ...
    def attack(self,
        ...
        adv_accs_1, adv_preds_1, victim_accs_1, victim_preds_1,  classes_use = perpoint_threshold_test_per_dist(
        ...
        adv_accs_2, adv_preds_2, victim_accs_2, victim_preds_2,  classes_use = perpoint_threshold_test_per_dist(
...
def _perpoint_threshold_on_ratio(
    ...
    if multi2:
        ...
        # Compute accuracy for given predictions,  and rules
        rs = np.repeat(np.expand_dims(rule,axis=1),combined.shape[1],axis=1)
        preds = np.mean(combined==rs,axis=0)>=0.5
        acc = np.mean(preds==classes)
...
def perpoint_threshold_test_per_dist(
    ...
    assert not config.loss_variant
    assert victim_preds_present
    assert not config.multi_class
    assert not epochwise_version
    assert not config.relative_threshold
    # Predictions by adversary's models
    p1 = preds_adv.preds_property_1.copy()>=0
    p2 = preds_adv.preds_property_2.copy()>=0

    if victim_preds_present:
        # Predictions by victim's models
        pv1 = preds_victim.preds_property_1.copy()>=0
        pv2 = preds_victim.preds_property_2.copy()>=0
    ...
    # Get rules
    rs = (np.mean(p1_,1)-np.mean(p2_,1))<=0
    ...
    for ratio in config.ratios:
        ...
        adv_acc, adv_pred = _perpoint_threshold_on_ratio(
            p1_use, p2_use, classes_adv,  rs_use,
        ...
        if victim_preds_present:
            ...
            if epochwise_version:
                ...
                victim_acc, victim_pred = _perpoint_threshold_on_ratio(
                    pv1_use, pv2_use, classes_victim, rs_use,
    ...
    return adv_accs, adv_preds, victim_accs, victim_preds, (classes_adv, classes_victim)
