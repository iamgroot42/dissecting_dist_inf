...
class Epoch_ThresholdAttack(Attack):
    ...
    def _thresh_per_dis(self,preds_vic,
        ...
        order =  epoch_order_p(preds_adv[0].preds_property_1,preds_adv[0].preds_property_2,preds_adv[1].preds_property_1,preds_adv[1].preds_property_2)
        adv_accs, f_accs= [],[]
        #pij: ith epoch, jth distribution
        p11,p12,p21,p22,y = np.transpose(preds_adv[0].preds_property_1)[order],np.transpose(preds_adv[0].preds_property_2)[order],np.transpose(preds_adv[1].preds_property_1)[order],np.transpose(preds_adv[1].preds_property_2)[order],ground_truth[order]
        pv11,pv12,pv21,pv22 = np.transpose(preds_vic[0].preds_property_1)[order],np.transpose(preds_vic[0].preds_property_2)[order],np.transpose(preds_vic[1].preds_property_1)[order],np.transpose(preds_vic[1].preds_property_2)[order]
        for ratio in self.config.ratios:
            ...
            advacc =  _acc_per_dis(PredictionsOnOneDistribution(p11[:leng],p12[:leng]),
                            PredictionsOnOneDistribution(p21[:leng],p22[:leng]),
                            y[:leng],
                            calc_acc,
                            t=True)
            vacc =  _acc_per_dis(PredictionsOnOneDistribution(pv11[:leng],pv12[:leng]),
                            PredictionsOnOneDistribution(pv21[:leng],pv22[:leng]),
                            y[:leng],
                            calc_acc,
                            t=True)
            if self.ratio:
