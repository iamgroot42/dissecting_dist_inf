...
class LossAndThresholdAttack(Attack):
    def attack(self,
        ...
        if epochwise_version:
            ...
            if multi2:
                basic = self._loss_multi(acc_1, acc_2,multi2)
            else:
    ...
    def _loss_multi(self, acc_1, acc_2,multi2:int):
        ...
        for r in range(len(self.config.ratios)):
            ...
            preds_2 = []
            for i in range(l):
                sampling = np.random.permutation(l)[:multi2]
                preds_1.append(np.mean(acc_1[0][r, sampling] > acc_2[0][r, sampling])>=0.5)
                preds_2.append(np.mean(acc_1[1][r, sampling] <= acc_2[1][r, sampling])>=0.5)
            basic.append(100*(np.mean(preds_1) + np.mean(preds_2)) / 2)
        return basic